...
function drawEntity(context, entity, borderWidth, bodyFillStyle, showName) {
    context.save();
    // border
    ...
    context.fillRect(entity.boundingBox.x + borderWidth, entity.boundingBox.y + borderWidth, entity.boundingBox.width - (borderWidth * 2), entity.boundingBox.height - (borderWidth * 2));
    if (showName) {
        // name text
        context.font = `${ENTITY_NAME_TEXT_SIZE_PX}px ${ENTITY_NAME_TEXT_FONT_FAMILY}`;
        context.fillStyle = ENTITY_NAME_TEXT_FILL_STYLE;
        context.fillText(entity.name, entity.boundingBox.x + borderWidth + (ENTITY_NAME_TEXT_SIZE_PX / 4), entity.boundingBox.y + borderWidth + ENTITY_NAME_TEXT_SIZE_PX);
    }
    context.restore();
}
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function drawFrame(context, state, fps) {
    ...
    state.entities.sequence()
        .filter(({ entity }) => (entity !== state.subject))
        .waitForEach(({ entity }) => {
